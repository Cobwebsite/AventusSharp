...
{
    public static string GetQuery(TableMemberInfoSql memberInfo, PostgreSqlStorage storage)
    {
        ...
        string intermediateName = storage.QuoteIdentifier(memberMultiple.TableIntermediateKey1 ?? "");
        ...
        string constraintProp = "\t" + "CONSTRAINT " + storage.QuoteIdentifier(constraintName) + " FOREIGN KEY (" + intermediateName + ") REFERENCES " + storage.QuoteIdentifier(instance.SqlTableName) + " (" + storage.QuoteIdentifier(instance.Primary.SqlName) + ") ON DELETE CASCADE";
        foreignConstraint.Add(constraintProp);



        intermediateName = storage.QuoteIdentifier(memberMultiple.TableIntermediateKey2 ?? "");
        ...
        if (addLinkConstraint)
        {
            ...
            constraintName = Utils.CheckConstraint(constraintName);
            constraintProp = "\t" + "CONSTRAINT " + storage.QuoteIdentifier(constraintName) + " FOREIGN KEY (" + intermediateName + ") REFERENCES " + storage.QuoteIdentifier(linkTableName) + " (" + storage.QuoteIdentifier(linkPrimaryName) + ") ON DELETE CASCADE";
            foreignConstraint.Add(constraintProp);
        }



        string sql = "CREATE TABLE " + storage.QuoteIdentifier(intermediateTableName) + " (\n";
        sql += string.Join(separator, schema);
        if (primaryConstraint.Count > 0)
        {
            sql += separator;
            string joinedPrimary = string.Join(",", primaryConstraint);
            string primaryProp = "\tCONSTRAINT " + storage.QuoteIdentifier("PK_" + intermediateTableName) + " PRIMARY KEY (" + joinedPrimary + ")";
            sql += primaryProp;
        }
