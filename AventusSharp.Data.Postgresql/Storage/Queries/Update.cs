...
public class Update
{
    public static DatabaseUpdateBuilderInfo PrepareSQL<X>(DatabaseUpdateBuilder<X> updateBuilder, PostgreSqlStorage storage) where X : IStorable
    {
    ...
    private static void LoadTableFieldUpdate(TableInfo tableInfo, string alias, DatabaseBuilderInfo baseInfo, List<ParamsInfo> updateParamsInfo, List<string> fields, PostgreSqlStorage storage)
    {
        foreach (TableMemberInfoSql member in tableInfo.Members)
        {
            ...
            if (member is ITableMemberInfoSqlWritable memberLink)
            {
                ...
                fields.Add(storage.QuoteIdentifier(member.SqlName) + " = @" + name);
            }
        }
